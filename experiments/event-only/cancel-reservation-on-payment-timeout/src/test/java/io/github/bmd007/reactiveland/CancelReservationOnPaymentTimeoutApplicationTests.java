...
class CancelReservationOnPaymentTimeoutApplicationTests {
    ...
    void contextLoads() {
        //given
        Flux<Boolean> booleanFlux = Flux.range(0, 6)
    ...
    private Mono<Boolean> reserveAndPayForTable() {
        ...
        return Mono.just(new Event.CustomerEvent.CustomerRequestedTable(customerId, tableId))
                ...
                .map(s -> s.equals("PAID_FOR"));
    }

    private Mono<Boolean> reserveTableAndLeave() {
        String customerId = UUID.randomUUID().toString();
        long delay = 22L;
        String tableId = UUID.randomUUID().toString();
        return Mono.just(new Event.CustomerEvent.CustomerRequestedTable(customerId, tableId))
                ...
                .map(TableReservationDto::status)
                .map(s -> s.equals("RESERVED_AWAITING_PAYMENT"));
    }

    private Mono<Boolean> reserveTableAndPayLate() {
        String customerId = UUID.randomUUID().toString();
        long delay = 20L;
        String tableId = UUID.randomUUID().toString();
        String paymentId = UUID.randomUUID().toString();
